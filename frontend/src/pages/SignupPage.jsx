...
// Safe fetch helper — always returns JSON, never throws on HTML error pages
async function safeFetch(url, options) {
  ...
  if (contentType.includes('application/json')) {
    ...
    // Server returned HTML (e.g. 404/500 page) — extract text for debugging
...
export default function SignupPage({ onNavigate, onSignupSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const { ok, data } = await safeFetch('http://localhost:5000/auth/signup', {
        method: 'POST',
      ...
      <div className="auth-card auth-card-signup">
        ...
        {step === 1 && (
          <div className="auth-step-content">
            ...
            {serverError && <div className="auth-error-banner">⚠️ {serverError}</div>}
        ...
        {step === 3 && (
          <div className="auth-step-content auth-success">
            ...
            <p className="auth-sub">
              Welcome to KidStar, <strong>{form.name}</strong>!<br />
              Time to create your child's adventure ✨
            </p>
            <div className="success-perks">
              {['✅ Zero ads, always', '✅ Unlimited stories & quizzes', "✅ Track your child's progress"].map((p, i) => (
                <div key={i} className="perk-item">{p}</div>
              ))}
            </div>
            <button
              className="auth-submit"
              onClick={() => onSignupSuccess?.({ name: form.name, email: form.email })}
            >
              Let's Go! 🚀
            </button>
          </div>
